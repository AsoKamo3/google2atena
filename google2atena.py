# -*- coding: utf-8 -*-
"""
Google連絡先CSV → 宛名職人CSV 変換（v3.9 full fix）
・会社名かな（発音ベース変換）
・電話番号 Work > Mobile > Home 優先統合
・住所分割
・メモ保持
・半角電話・全角住所統一
"""
...
def company_to_kana(name: str) -> str:
    ...
    # 日本語が含まれている場合
    if re.search(r"[ぁ-んァ-ン一-龥]", name):
        name = name.translate(str.maketrans({chr(i): chr(i + 0x60) for i in range(ord("ぁ"), ord("ゖ"))}))
        return name
    ...
    romaji_map = {
        "MAIN": "メイン", "TENT": "テント",
        "PHP": "ピーエイチピー", "PRESS": "プレス",
        "BOOK": "ブック", "NEWS": "ニュース",
        "FOUNDATION": "ファウンデーション", "CORPORATION": "コーポレーション",
    }
    ...
    return "".join(letter_map.get(ch, ch) for ch in name_upper)
...
def merge_phones(row):
    ...
    for label in ["Work", "Mobile", "Home"]:
        for i in range(1, 6):
            ...
            if label.lower() in lbl.lower() or label.lower() in value.lower():
...
def convert_csv(input_csv):
    ...
    for _, row in df.iterrows():
        ...
        addr1 = unicodedata.normalize("NFKC", addr1)
        addr2 = unicodedata.normalize("NFKC", addr2)
...
def index():
    ...
    <h2>Google連絡先CSV → 宛名職人CSV 変換（v3.9 full fix）</h2>
...
def convert():
    ...
    return send_file(buffer, as_attachment=True, download_name="converted_v39.csv", mimetype="text/csv")
